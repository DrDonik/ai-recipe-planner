
export const translations = {
    English: {
        ingredients: "Ingredients",
        instructions: "Instructions",
        needToBuy: "Need to Buy",
        shoppingList: "Shopping List",
        /** Brand name — identical in every locale, but kept here so the visible
         *  heading and the accessible names that quote it cannot drift apart. */
        appTitle: "AI Recipe Planner",
        skipToContent: "Skip to main content",
        a11y: {
            expand: "Expand",
            collapse: "Collapse",
            close: "Close",
            info: "Info",
        },
        generate: "Generate Meal Plan",
        planning: "Planning Meals...",
        cancel: "Cancel",
        cancelGeneration: "Cancel meal plan generation",
        ready: "Ready to plan?",
        readyDesc: "Add your ingredients, set the number of people, and let AI design a minimal-waste menu for you.",
        people: "People to feed",
        diet: "Diet",
        language: "Language",
        menuTitle: "Your Personalized Menu",
        pantry: "Pantry Inventory",
        add: "Add",
        remove: "Remove",
        clickToEdit: "Click to edit",
        noVeg: "No ingredients added yet.",
        apiKeyError: "Please enter a valid API Key. Get one at: https://aistudio.google.com/apikey",
        veggieError: "Please add at least one ingredient to your pantry.",
        invalidSharedData: "Invalid shared data. The link may be corrupted.",
        generateError: "Something went wrong generating recipes.",
        parseError: "Something went wrong parsing the response.",
        storageError: "Could not save data — your browser storage may be full. Some changes may be lost on refresh.",
        errors: {
            invalidStructure: "Invalid recipe data structure",
            tryAgain: "Please try generating recipes again.",
            invalidJson: "Failed to parse recipe data. The AI returned invalid JSON.",
            apiKeyRequired: "API Key is required",
            fetchFailed: "Failed to fetch recipes",
            emptyResponse: "No recipes generated. The AI returned an empty response.",
            timeout: "Request timed out. Please try again.",
            planTimeout: "Timed out. Creating the meal plan took too long. Try again, or create fewer recipes at once.",
            networkError: "Network error. Please check your internet connection.",
            unexpectedError: "An unexpected error occurred. Please try again.",
            imageBlocked: "Image generation was blocked by the safety filter.",
            imageNoData: "The image model returned no image. Please try again.",
            imageQuotaExceeded: "Image generation quota exceeded. Please try again later.",
            imageFreeTierUnsupported: "Image generation isn't available on the free Gemini tier. The image buttons stay hidden until you save a different key.",
            chatBlocked: "The assistant couldn't answer that. Please try rephrasing your question.",
            chatQuotaExceeded: "Chat quota exceeded. Please try again later.",
        },
        identifyIngredient: {
            buttonAriaLabel: "Identify ingredient from photo",
            cancelAriaLabel: "Cancel identification",
            identifying: "Identifying…",
            unknown: "Couldn't identify the ingredient — please type it.",
            multiple: "Photo shows multiple ingredients — please photograph one at a time.",
            error: "Couldn't identify the photo. Please try again or type it.",
            decodeFailed: "Couldn't read the photo on this device.",
            quotaExceeded: "Photo identification quota exceeded. Please try again later.",
        },
        ownRecipe: {
            title: "Your own recipe",
            heading: "Your own recipes",
            cameraAriaLabel: "Photograph a recipe",
            cancelRead: "Stop reading",
            writeAriaLabel: "Write out a recipe",
            transcribing: "Reading the recipe…",
            transcribed: "Recipe read. Please check it.",
            truncated: "The page was longer than could be read in one go — this recipe is cut off. Photograph the rest to add it.",
            entryFull: "This entry is full, so the photo was not added and the text below is unchanged. Shorten it, then photograph again.",
            tooLongForEntry: "The recipe is longer than one entry can hold and the end is missing. Split it across two entries if you need all of it.",
            placeholder: "Title, servings, ingredients, preparation — one per line.",
            hint: "It is planned as one of your meals, rescaled to your number of people, and kept as written even where it differs from your diet.",
            addPhoto: "Add a photo",
            add: "Add recipe",
            previewAriaLabel: "Show recipe",
            removeAriaLabel: "Remove recipe",
            unreadable: "No recipe found in the photo. Try again or write it out.",
            error: "Couldn't read the photo. Please try again or write the recipe out.",
            decodeFailed: "Couldn't read the photo on this device.",
            quotaExceeded: "Photo quota exceeded. Please try again later.",
        },
        photoPrivacy: {
            title: "Your photo will be sent to Google",
            description: "To identify an ingredient, the picture is uploaded to Google Gemini, which returns the ingredient name.",
            beforeYouSend: "Before you send:",
            note1: "The photo leaves your device and is processed on Google's servers.",
            note2: "Make sure no people, documents or anything else are in the frame.",
            usage: "This app does not store the photo, but Google's terms and data handling apply.",
            recipeDescription: "To read a recipe, the picture is uploaded to Google Gemini, which returns the recipe as text.",
            recipeNote2: "Photograph the recipe alone — leave out private notes, addresses and anything else on the page.",
            send: "Send photo",
        },
        meals: "Meals to plan",
        dietOptions: {
            vegan: "Vegan",
            vegetarian: "Vegetarian",
            mostlyVegetarian: "Mostly Vegetarian",
            pescatarian: "Pescatarian",
            flexitarian: "Flexitarian",
            omnivore: "Omnivore",
            carnivore: "Carnivore",
        },
        styleWishes: "Style, Wishes, etc.",
        styleWishesPlaceholder: "Indian, Gluten-free, ...",
        noStyleWishes: "No style preferences added yet.",
        plannedRecipes: "Requested Dishes",
        plannedRecipesPlaceholder: "Mushroom risotto, ...",
        noPlannedRecipes: "No requested dishes yet — meals are planned freely.",
        spiceRack: "Spice Rack & Staples",
        spicesPlaceholder: "Add spices, oils, sauces...",
        kitchenAppliances: "Kitchen Appliances",
        appliancesPlaceholder: "Add air fryer, Instant Pot...",
        kitchenAppliancesInfo: "Add appliances like an air fryer or Vitamix. The AI is aware of them and may suggest recipes that use them.",
        noAppliances: "No appliances added yet.",
        kitchen: {
            caption: "My Kitchen",
            info: "Spice rack & staples and kitchen appliances belong to the selected kitchen. Create a second kitchen (e.g. for a holiday home) and switch between them here.",
            switchAria: "Switch kitchen",
            defaultName: "Home",
            newKitchen: "New kitchen…",
            namePlaceholder: "Kitchen name",
            createEmpty: "Empty",
            createCopy: "Copy",
            editAria: "Edit kitchen",
            deleteAria: "Delete kitchen",
            confirmDelete: "Delete?",
            saveAria: "Save name",
            locationLabel: "Location",
            locationPlaceholder: "Town or city",
            locationSearching: "Searching…",
            locationNoResults: "No matching place found.",
            /* Announced to screen readers only, prefixed with the count. */
            locationResultsOne: "place found.",
            locationResultsMany: "places found.",
            locationClearAria: "Remove location",
            weatherAttribution: "Weather data: Open-Meteo (CC BY 4.0)",
            weatherTooltip: "Daytime highs for the coming days at this location. Used as a gentle hint when planning the recipes.",
            weatherChangeable: "changeable",
            weatherConditions: {
                clear: "mostly sunny",
                cloudy: "mostly cloudy",
                rain: "rainy",
                snow: "snowy",
                storm: "stormy",
                fog: "foggy",
            },
        },
        placeholders: {
            ingredient: "Ingredient (e.g. Carrots)",
            amount: "Amount (e.g. 1kg or 3 pcs)",
        },
        pantryInfo: "Add your available ingredients here. The AI will try to use all of them in the generated recipes.",
        spiceRackInfo: "Add staples like spices, oils, or salt. These guide the recipes but won't be added to your shopping list.",
        sharedPantry: "Shared Pantry",
        loadPantry: "Load these items into my pantry",
        noPantryToShare: "No pantry items to share!",
        openFocusView: "Open focus view",
        openMyPlanner: "Open my AI Recipe Planner",
        sharePantry: "Share pantry",
        sharePantrySuccess: "Pantry URL copied to clipboard!",
        noSpices: "No spices added yet.",
        apiKeyPlaceholder: "Paste Gemini API Key",
        apiKeyLabel: "Gemini API Key",
        getApiKey: "Get API key",
        languageLabel: "Select language",
        decreasePeople: "Decrease people count",
        increasePeople: "Increase people count",
        decreaseMeals: "Decrease meals count",
        increaseMeals: "Increase meals count",
        decoupled: "(decoupled)",
        decoupledInfo: "This shopping list was shared with you. Your checkmarks are saved only on this device and won't affect your own list.",
        keepScreenOn: "Keep screen on",
        screenKeptOn: "Screen kept on",
        deleteRecipe: "Delete recipe",
        timers: {
            title: "Timers",
            startAria: "tap to start a timer",
            done: "Done!",
            followUp: "Extra time",
            pause: "Pause",
            resume: "Resume",
            cancel: "Cancel timer",
            dismiss: "Dismiss",
            mute: "Mute timer sound",
            unmute: "Unmute timer sound",
        },
        recipeImage: {
            generate: "Generate image",
            generating: "Generating image…",
            retry: "Retry",
            remove: "Remove image",
        },
        replaceRecipe: {
            button: "Replace recipe",
            title: "Replace recipe",
            replacing: "Replacing:",
            prompt: "What would you prefer instead?",
            placeholder: "e.g. something lighter, with chicken, Asian…",
            hint: "Leave empty to simply get a different recipe.",
            submit: "Replace",
            generating: "Finding a new recipe…",
            cancelGeneration: "Cancel replacement",
        },
        recipeChat: {
            open: "Ask about this recipe",
            close: "Close chat",
            title: "Kitchen Assistant",
            intro: "Ask questions about this recipe.",
            placeholder: "Ask something…",
            send: "Send",
            clear: "Clear chat",
            thinking: "Thinking…",
            retry: "Retry",
            suggestions: [
                "How long does this take?",
                "At what temperature?",
                "Any pro tips?",
                "Can this be refined?",
                "Can anything be prepared ahead?",
            ],
        },
        imageGenCost: {
            title: "Recipe images cost money",
            description: "Each image is drawn by Google Gemini with your stored API key and billed to your Google account.",
            beforeYouStart: "Before the first image:",
            note1: "Image generation needs a paid Gemini tier — a free-tier key cannot do it.",
            note2: "Every image is charged separately, including one you generate again.",
            usage: "The recipe's title and ingredients are sent to Google Gemini to draw the picture. Images stay on this device: they are never synced and never part of a shared link.",
            generate: "Generate image",
        },
        emptyPantry: "Empty Pantry",
        clearShoppingList: "Clear Shopping List",
        servings: { one: "serving", other: "servings" },
        nutritionPerServing: "Per serving (estimate)",
        calories: "kcal",
        carbs: "Carbs",
        fat: "Fat",
        protein: "Protein",
        welcome: {
            title: "Welcome to AI Recipe Planner",
            intro: "This app helps you plan meals using AI, minimizing food waste by using what you already have.",
            pantry: "Add your available ingredients to the Pantry. The AI will try to use all of them in your recipes.",
            spiceRack: "The Spice Rack stores staples like spices and oils that you always have on hand.",
            settings: "Choose your diet preference, number of people, and how many meals to plan.",
            copyPasteMode: "Use Copy & Paste mode (default) to work with any AI chat (ChatGPT, Claude, Gemini, etc.) without storing credentials.",
            apiKey: "A Gemini API key generates meal plans directly, and also unlocks photo recognition, storage tips, recipe images, single-recipe replacement and the cooking chat. Get one at Google AI Studio.",
            generates: "Hit 'Generate' and the AI will create personalized recipes with a consolidated shopping list.",
            localStorage: "Your settings and recipes are saved in your browser's local storage. If you clear your browser data, this information will be lost.",
            dontShowAgain: "Don't show this again",
            getStarted: "Get Started"
        },
        modeSwitch: {
            label: "Generate directly",
            tooltip: "Off: the meal-plan prompt is prepared for any AI chat. On: the meal plan is generated directly with your stored Gemini API key."
        },
        storageTips: {
            iconAriaLabel: "Storage tip",
            popoverTitle: "Storage tip",
            loading: "Fetching storage tip…",
        },
        copyPaste: {
            title: "Copy & Paste",
            description: "Use any AI chat instead of an API key",
            step1Description: "Copy the prompt below to use with your favorite AI",
            step2Description: "Paste the AI's response here",
            stepCopy: "Copy Prompt",
            stepPaste: "Paste Response",
            copyPrompt: "Copy Prompt",
            copied: "Copied!",
            next: "Next",
            back: "Back",
            submit: "Import Recipes",
            instructions: "Click 'Copy Prompt' below. Then open any AI chat (ChatGPT, Claude, Gemini, etc.), paste the prompt and send it. Once you get the AI's response, copy it (use the copy icon",
            instructionsEnd: "below the response), come back here, and click 'Import Recipes'.",
            instructionsPasteIntro: "Prompt copied successfully. Follow these steps to get the AI's response:",
            instructionsPasteStep1: "Open any AI chat (ChatGPT, Claude, Gemini, etc.), paste the prompt and send it",
            instructionsPasteStep2: "Copy the AI's response — use the copy icon",
            instructionsPasteStep2End: "below the response",
            instructionsPasteStep3: "Come back here and click 'Import Recipes'",
            responsePlaceholder: "Paste the exact AI response here...",
            responseRequired: "No AI response found in your clipboard. Please paste it manually into the field above.",
            clipboardIsPrompt: "You didn't copy the AI's response yet. Paste the prompt into your AI chat, copy the response, and try again.",
            copyFailed: "Couldn't copy automatically. Please select the text above and copy it manually."
        },
        apiKeySecurity: {
            title: "Your API key will be stored in your browser",
            description: "Your API key will be stored in your browser's local storage in plain text.",
            risks: "This means:",
            risk1: "Anyone with access to this device can view your API key",
            risk2: "Browser extensions may be able to read it",
            risk3: "The key persists until you clear browser data",
            risk4: "The key will be visible in network requests (browser DevTools, network logs, proxies)",
            usage: "The key is only used to call Google Gemini directly from your browser — for meal plans, photo recognition, storage tips, images, replacement and chat. No request passes through a server of this app.",
            understand: "I understand, continue"
        },
        apiKeyDialog: {
            heading: "Gemini API key",
            description: "The key is stored in this browser and sent straight to Google Gemini. It powers photo recognition, storage tips, recipe images, single-recipe replacement and the cooking chat; saving it also switches meal-plan generation over to the API, which you can switch back at any time.",
            save: "Save key",
            delete: "Delete key",
            errorEmpty: "Please enter an API key.",
            editTooltip: "Change API key",
            tooltip: "Off: no key is stored, and meal plans are prepared for copy & paste. On: the key is stored in this browser and enables direct generation, photo recognition, storage tips, recipe images, single-recipe replacement and the cooking chat."
        },
        clearApiKey: {
            title: "Clear API Key?",
            description: "Photo recognition, storage tips, recipe images, single-recipe replacement and the cooking chat stop being offered, and meal plans go back to copy & paste. You can undo this for a few seconds.",
            clear: "Yes, clear it",
            keep: "No, keep it"
        },
        undo: {
            pantryEmptied: "Pantry emptied",
            shoppingListCleared: "Shopping list cleared",
            recipeDeleted: "Recipe deleted",
            apiKeyCleared: "API key cleared",
            mealPlanReplaced: "New meal plan created",
            action: "Undo"
        },
        aiDisclaimer: "The recipes are generated directly by AI. AI can make mistakes. Always cook with common sense.",
        dataTransfer: {
            exportData: "Export data",
            importData: "Import data",
            exportSuccess: "Data exported successfully!",
            importSuccess: "Data imported successfully! Reloading...",
            invalidImportStructure: "Invalid backup file. The file structure is not recognized.",
            invalidImportJson: "Invalid file. Please select a valid JSON backup file.",
            importReadError: "Could not read the file. Please try again.",
            importConfirm: "This will overwrite all your current data (pantry, spices, recipes, settings). Continue?",
        },
        sync: {
            title: "Multi-Device Sync",
            label: "Device sync",
            openSettings: "Sync settings",
            enabledTooltip: "Sync is active",
            pendingTooltip: "Changes will sync shortly…",
            pushingTooltip: "Syncing to GitHub…",
            pullingTooltip: "Loading from GitHub…",
            disabledTooltip: "Sync is off",
            errorTooltip: "Sync error — check sync settings",
            tooltip: "Off: nothing leaves this device. On: recipes, pantry and settings are kept in step across your devices through a private GitHub Gist.",
            editTooltip: "Change sync settings",
            securityTitle: "Your GitHub token will be stored in your browser",
            securityDescription: "To sync across devices, your GitHub Personal Access Token will be stored in your browser's local storage in plain text.",
            securityRisks: "Please understand:",
            securityRisk1: "Only create a token with the 'gist' scope — nothing more.",
            securityRisk2: "Anyone with access to this device can read the token.",
            securityRisk3: "Browser extensions may be able to read it.",
            securityRisk4: "Revoke the token on GitHub if this device is lost or shared.",
            securityRecommendation: "The token is only used to read and write a single private Gist holding your recipe data.",
            securityAccept: "I understand, continue",
            setupHeading: "Set up sync",
            setupDescription: "Paste a GitHub Personal Access Token with the 'gist' scope. If you already have a sync Gist from another device, enter its ID too; otherwise leave it empty and a new Gist will be created.",
            tokenLabel: "Personal Access Token",
            tokenPlaceholder: "ghp_…",
            gistIdLabel: "Gist ID (optional)",
            gistIdPlaceholder: "e.g. a1b2c3d4e5f6…",
            getTokenLink: "Create a token on GitHub",
            createNew: "Create new Gist",
            useExisting: "Use existing Gist",
            creating: "Creating Gist…",
            connecting: "Connecting…",
            activeHeading: "Sync is active",
            activeGistId: "Gist ID",
            copyGistId: "Copy Gist ID",
            gistIdCopied: "Gist ID copied to clipboard",
            disableAndClear: "Switch off and delete token",
            disableAndKeep: "Switch off and keep token",
            disableNote: "Your Gist stays on GitHub either way — switching off only affects this device. Deleting the token removes it from this browser for good: there is no undo, and you would have to enter it again to sync here.",
            storedHeading: "Sync is off, token still stored",
            storedNote: "Sync is switched off on this device, but your GitHub token is still in this browser's local storage. Delete it if you do not plan to sync here again — there is no undo.",
            storedClear: "Delete token",
            storedKeep: "Keep token",
            tokenStoredWarning: "GitHub token stored in local storage",
            pulledNotification: "Synced from another device",
            errorUnauthorized: "Sync error: token is unauthorized or revoked. Please update it.",
            errorNotFound: "Sync error: the Gist was not found. It may have been deleted on GitHub.",
            errorPayload: "Sync error: the Gist content is malformed. It may have been edited outside the app.",
            errorNetwork: "Sync error: could not reach GitHub. Please try again later.",
            errorTokenEmpty: "Please enter a token.",
            errorGistIdEmpty: "Please enter a Gist ID.",
        },
    },
    German: {
        ingredients: "Zutaten",
        instructions: "Zubereitung",
        needToBuy: "Einkaufen",
        shoppingList: "Einkaufsliste",
        appTitle: "AI Recipe Planner",
        skipToContent: "Zum Hauptinhalt springen",
        a11y: {
            expand: "Ausklappen",
            collapse: "Einklappen",
            close: "Schließen",
            info: "Info",
        },
        generate: "Mahlzeiten planen",
        planning: "Plane Gerichte...",
        cancel: "Abbrechen",
        cancelGeneration: "Mahlzeitenplanung abbrechen",
        ready: "Bereit zum Planen?",
        readyDesc: "Füge deine Zutaten hinzu, wähle die Personenanzahl und lass die KI einen Menüplan erstellen.",
        people: "Personenanzahl",
        diet: "Ernährung",
        language: "Sprache",
        menuTitle: "Dein Persönliches Menü",
        pantry: "Vorratsschrank",
        add: "Hinzufügen",
        remove: "Entfernen",
        clickToEdit: "Zum Bearbeiten klicken",
        noVeg: "Noch keine Zutaten hinzugefügt.",
        apiKeyError: "Bitte gib einen gültigen API-Key ein. Hol dir einen auf: https://aistudio.google.com/apikey",
        veggieError: "Bitte füge mindestens eine Zutat zu deinem Vorrat hinzu.",
        invalidSharedData: "Ungültige geteilte Daten. Der Link könnte beschädigt sein.",
        generateError: "Beim Generieren der Rezepte ist ein Fehler aufgetreten.",
        parseError: "Beim Verarbeiten der Antwort ist ein Fehler aufgetreten.",
        storageError: "Daten konnten nicht gespeichert werden — Ihr Browserspeicher ist möglicherweise voll. Einige Änderungen könnten beim Neuladen verloren gehen.",
        errors: {
            invalidStructure: "Ungültige Rezeptdatenstruktur",
            tryAgain: "Bitte versuche erneut, Rezepte zu generieren.",
            invalidJson: "Rezeptdaten konnten nicht verarbeitet werden. Die KI hat ungültiges JSON zurückgegeben.",
            apiKeyRequired: "API-Schlüssel erforderlich",
            fetchFailed: "Rezepte konnten nicht abgerufen werden",
            emptyResponse: "Keine Rezepte generiert. Die KI hat eine leere Antwort zurückgegeben.",
            timeout: "Zeitüberschreitung. Bitte versuche es erneut.",
            planTimeout: "Zeitüberschreitung. Die Erstellung des Menüplans hat zu lange gedauert. Versuche es erneut oder erstelle weniger Rezepte auf einmal.",
            networkError: "Netzwerkfehler. Bitte überprüfe deine Internetverbindung.",
            unexpectedError: "Ein unerwarteter Fehler ist aufgetreten. Bitte versuche es erneut.",
            imageBlocked: "Die Bildgenerierung wurde vom Sicherheitsfilter blockiert.",
            imageNoData: "Das Bildmodell hat kein Bild zurückgegeben. Bitte versuche es erneut.",
            imageQuotaExceeded: "Bildgenerierungs-Kontingent erschöpft. Bitte später erneut versuchen.",
            imageFreeTierUnsupported: "Bildgenerierung ist im kostenlosen Gemini-Tier nicht verfügbar. Die Bild-Schaltflächen bleiben ausgeblendet, bis du einen anderen Schlüssel speicherst.",
            chatBlocked: "Die Küchenhilfe konnte darauf nicht antworten. Formuliere die Frage bitte anders.",
            chatQuotaExceeded: "Chat-Kontingent erschöpft. Bitte später erneut versuchen.",
        },
        identifyIngredient: {
            buttonAriaLabel: "Zutat per Foto erkennen",
            cancelAriaLabel: "Erkennung abbrechen",
            identifying: "Erkenne…",
            unknown: "Zutat konnte nicht erkannt werden — bitte tippe sie ein.",
            multiple: "Das Foto zeigt mehrere Zutaten — bitte fotografiere jeweils nur eine.",
            error: "Foto konnte nicht ausgewertet werden. Bitte erneut versuchen oder eintippen.",
            decodeFailed: "Das Foto konnte auf diesem Gerät nicht gelesen werden.",
            quotaExceeded: "Kontingent für die Foto-Erkennung erschöpft. Bitte später erneut versuchen.",
        },
        ownRecipe: {
            title: "Eigenes Rezept",
            heading: "Eigene Rezepte",
            cameraAriaLabel: "Rezept fotografieren",
            cancelRead: "Lesen abbrechen",
            writeAriaLabel: "Rezept selbst eintippen",
            transcribing: "Lese das Rezept…",
            transcribed: "Rezept gelesen. Bitte prüfen.",
            truncated: "Die Seite war länger als in einem Zug lesbar — dieses Rezept ist abgeschnitten. Fotografiere den Rest, um ihn zu ergänzen.",
            entryFull: "Dieser Eintrag ist voll, das Foto wurde nicht übernommen und der Text unten ist unverändert. Kürze ihn und fotografiere erneut.",
            tooLongForEntry: "Das Rezept ist länger, als ein Eintrag fassen kann, das Ende fehlt. Teile es auf zwei Einträge auf, wenn du alles brauchst.",
            placeholder: "Titel, Portionen, Zutaten, Zubereitung — je Zeile eines.",
            hint: "Es wird als eine deiner Mahlzeiten geplant, auf deine Personenzahl umgerechnet und bleibt so, wie es dasteht, auch wenn es von deiner Ernährungsweise abweicht.",
            addPhoto: "Foto hinzufügen",
            add: "Rezept hinzufügen",
            previewAriaLabel: "Rezept anzeigen",
            removeAriaLabel: "Rezept entfernen",
            unreadable: "Auf dem Foto war kein Rezept zu erkennen. Erneut versuchen oder eintippen.",
            error: "Das Foto konnte nicht ausgewertet werden. Bitte erneut versuchen oder eintippen.",
            decodeFailed: "Das Foto konnte auf diesem Gerät nicht gelesen werden.",
            quotaExceeded: "Foto-Kontingent erschöpft. Bitte später erneut versuchen.",
        },
        photoPrivacy: {
            title: "Dein Foto wird an Google gesendet",
            description: "Zum Erkennen der Zutat wird das Bild an Google Gemini hochgeladen, das den Namen der Zutat zurückgibt.",
            beforeYouSend: "Bevor du sendest:",
            note1: "Das Foto verlässt dein Gerät und wird auf Googles Servern verarbeitet.",
            note2: "Achte darauf, dass keine Personen, Dokumente oder anderes mit im Bild sind.",
            usage: "Diese App speichert das Foto nicht, es gelten aber Googles Bedingungen und Datenverarbeitung.",
            recipeDescription: "Zum Lesen des Rezepts wird das Bild an Google Gemini hochgeladen, das das Rezept als Text zurückgibt.",
            recipeNote2: "Fotografiere nur das Rezept — private Notizen, Adressen und anderes auf der Seite gehören nicht ins Bild.",
            send: "Foto senden",
        },
        meals: "Anzahl Mahlzeiten",
        dietOptions: {
            vegan: "Vegan",
            vegetarian: "Vegetarisch",
            mostlyVegetarian: "Größtenteils Vegetarisch",
            pescatarian: "Pescetarisch",
            flexitarian: "Flexitarisch",
            omnivore: "Omnivor",
            carnivore: "Fleischlastig",
        },
        styleWishes: "Stil, Wünsche, etc.",
        styleWishesPlaceholder: "Indisch, Glutenfrei, ...",
        noStyleWishes: "Noch keine Stil-Präferenzen hinzugefügt.",
        plannedRecipes: "Gewünschte Gerichte",
        plannedRecipesPlaceholder: "Risotto mit Pilzen, ...",
        noPlannedRecipes: "Noch keine gewünschten Gerichte — Mahlzeiten werden frei geplant.",
        spiceRack: "Gewürzregal & Grundvorräte",
        spicesPlaceholder: "Gewürze, Öl, Soßen hinzufügen...",
        kitchenAppliances: "Küchengeräte",
        appliancesPlaceholder: "Heißluftfritteuse, Instant Pot...",
        kitchenAppliancesInfo: "Füge Geräte wie eine Heißluftfritteuse oder einen Vitamix hinzu. Die KI kennt sie und kann Rezepte vorschlagen, die sie nutzen.",
        noAppliances: "Noch keine Geräte hinzugefügt.",
        kitchen: {
            caption: "Meine Küche",
            info: "Gewürzregal & Grundvorräte sowie Küchengeräte gehören zur ausgewählten Küche. Lege eine zweite Küche an (z. B. fürs Ferienhaus) und wechsle hier zwischen ihnen.",
            switchAria: "Küche wechseln",
            defaultName: "Zuhause",
            newKitchen: "Neue Küche…",
            namePlaceholder: "Name der Küche",
            createEmpty: "Leer",
            createCopy: "Kopie",
            editAria: "Küche bearbeiten",
            deleteAria: "Küche löschen",
            confirmDelete: "Löschen?",
            saveAria: "Name speichern",
            locationLabel: "Ort",
            locationPlaceholder: "Stadt oder Ort",
            locationSearching: "Suche…",
            locationNoResults: "Kein passender Ort gefunden.",
            locationResultsOne: "Ort gefunden.",
            locationResultsMany: "Orte gefunden.",
            locationClearAria: "Ort entfernen",
            weatherAttribution: "Wetterdaten: Open-Meteo (CC BY 4.0)",
            weatherTooltip: "Tageshöchstwerte der kommenden Tage an diesem Ort. Fließen als sanfter Hinweis in die Rezeptplanung ein.",
            weatherChangeable: "wechselhaft",
            weatherConditions: {
                clear: "meist sonnig",
                cloudy: "meist bewölkt",
                rain: "regnerisch",
                snow: "schneereich",
                storm: "stürmisch",
                fog: "neblig",
            },
        },
        placeholders: {
            ingredient: "Zutat (z.B. Karotten)",
            amount: "Menge (z.B. 1kg oder 3 Stück)",
        },
        pantryInfo: "Füge hier deine verfügbaren Zutaten hinzu. Die KI wird versuchen, alle davon in den Rezepten zu verwenden.",
        spiceRackInfo: "Füge Grundvorräte wie Gewürze, Öle oder Salz hinzu. Diese leiten die Rezepte, werden aber nicht auf die Einkaufsliste gesetzt.",
        sharedPantry: "Geteilter Vorratsschrank",
        loadPantry: "Diese Zutaten in meinen Vorrat laden",
        noPantryToShare: "Keine Vorratszutaten zum Teilen!",
        openFocusView: "Fokusansicht öffnen",
        openMyPlanner: "Meinen AI Recipe Planner öffnen",
        sharePantry: "Vorrat teilen",
        sharePantrySuccess: "Vorrats-URL in Zwischenablage kopiert!",
        noSpices: "Noch keine Gewürze hinzugefügt.",
        apiKeyPlaceholder: "Gemini API-Schlüssel einfügen",
        apiKeyLabel: "Gemini API-Schlüssel",
        getApiKey: "API-Schlüssel holen",
        languageLabel: "Sprache auswählen",
        decreasePeople: "Personenanzahl verringern",
        increasePeople: "Personenanzahl erhöhen",
        decreaseMeals: "Mahlzeitenanzahl verringern",
        increaseMeals: "Mahlzeitenanzahl erhöhen",
        decoupled: "(entkoppelt)",
        decoupledInfo: "Diese Einkaufsliste wurde mit dir geteilt. Deine Häkchen werden nur auf diesem Gerät gespeichert und beeinflussen deine eigene Liste nicht.",
        keepScreenOn: "Bildschirm anlassen",
        screenKeptOn: "Bildschirm bleibt an",
        deleteRecipe: "Rezept löschen",
        timers: {
            title: "Timer",
            startAria: "tippen, um einen Timer zu starten",
            done: "Fertig!",
            followUp: "Nachtimer",
            pause: "Pause",
            resume: "Fortsetzen",
            cancel: "Timer abbrechen",
            dismiss: "Schließen",
            mute: "Timer-Ton stummschalten",
            unmute: "Timer-Ton einschalten",
        },
        recipeImage: {
            generate: "Bild generieren",
            generating: "Bild wird generiert…",
            retry: "Erneut versuchen",
            remove: "Bild entfernen",
        },
        replaceRecipe: {
            button: "Rezept ersetzen",
            title: "Rezept ersetzen",
            replacing: "Wird ersetzt:",
            prompt: "Was möchtest du stattdessen?",
            placeholder: "z. B. etwas Leichteres, mit Hähnchen, asiatisch…",
            hint: "Leer lassen, um einfach ein anderes Rezept zu erhalten.",
            submit: "Ersetzen",
            generating: "Neues Rezept wird gesucht…",
            cancelGeneration: "Ersetzen abbrechen",
        },
        recipeChat: {
            open: "Zum Rezept fragen",
            close: "Chat schließen",
            title: "Küchenhilfe",
            intro: "Fragen zum Rezept stellen.",
            placeholder: "Frag etwas…",
            send: "Senden",
            clear: "Chat leeren",
            thinking: "Denkt nach…",
            retry: "Nochmal",
            suggestions: [
                "Wie lange dauert das?",
                "Bei welcher Temperatur?",
                "Irgendwelche Profi-Tipps?",
                "Kann man das verfeinern?",
                "Kann man etwas vorbereiten?",
            ],
        },
        imageGenCost: {
            title: "Rezeptbilder kosten Geld",
            description: "Jedes Bild wird von Google Gemini mit deinem gespeicherten API-Schlüssel erzeugt und deinem Google-Konto verrechnet.",
            beforeYouStart: "Vor dem ersten Bild:",
            note1: "Bildgenerierung braucht ein kostenpflichtiges Gemini-Kontingent — mit einem Gratis-Schlüssel geht es nicht.",
            note2: "Jedes Bild wird einzeln verrechnet, auch ein erneut erzeugtes.",
            usage: "Titel und Zutaten des Rezepts werden an Google Gemini gesendet, um das Bild zu zeichnen. Die Bilder bleiben auf diesem Gerät: sie werden nicht synchronisiert und sind nie Teil eines geteilten Links.",
            generate: "Bild erzeugen",
        },
        emptyPantry: "Vorrat leeren",
        clearShoppingList: "Einkaufsliste leeren",
        servings: { one: "Portion", other: "Portionen" },
        nutritionPerServing: "Pro Portion (Schätzung)",
        calories: "kcal",
        carbs: "Kohlenhydrate",
        fat: "Fett",
        protein: "Eiweiß",
        welcome: {
            title: "Willkommen beim AI Recipe Planner",
            intro: "Diese App hilft dir, Mahlzeiten mit KI zu planen und Lebensmittelverschwendung zu minimieren.",
            pantry: "Füge deine verfügbaren Zutaten zum Vorratsschrank hinzu. Die KI versucht, alle davon zu verwenden.",
            spiceRack: "Das Gewürzregal speichert Grundvorräte wie Gewürze und Öle, die du immer da hast.",
            settings: "Wähle deine Ernährungsvorlieben, Personenanzahl und wie viele Mahlzeiten geplant werden sollen.",
            copyPasteMode: "Nutze den Kopieren & Einfügen-Modus (Standard), um mit jedem KI-Chat (ChatGPT, Claude, Gemini, etc.) ohne gespeicherte Zugangsdaten zu arbeiten.",
            apiKey: "Ein Gemini-API-Schlüssel erzeugt Menüpläne direkt und schaltet zusätzlich Fotoerkennung, Lagerungstipps, Rezeptbilder, das Ersetzen einzelner Rezepte und den Kochchat frei. Hole dir einen bei Google AI Studio.",
            generates: "Drücke 'Planen' und die KI erstellt personalisierte Rezepte mit einer zusammengefassten Einkaufsliste.",
            localStorage: "Deine Einstellungen und Rezepte werden im lokalen Speicher deines Browsers gespeichert. Wenn du deine Browserdaten löschst, gehen diese Informationen verloren.",
            dontShowAgain: "Nicht mehr anzeigen",
            getStarted: "Los geht's"
        },
        modeSwitch: {
            label: "Direkt generieren",
            tooltip: "Aus: der Prompt für den Menüplan wird für einen beliebigen KI-Chat vorbereitet. An: der Menüplan wird direkt mit deinem gespeicherten Gemini-API-Schlüssel erzeugt."
        },
        storageTips: {
            iconAriaLabel: "Lagerungstipp",
            popoverTitle: "Lagerungstipp",
            loading: "Lade Lagerungstipp…",
        },
        copyPaste: {
            title: "Kopieren & Einfügen",
            description: "Verwende einen beliebigen KI-Chat anstelle eines API-Schlüssels",
            step1Description: "Kopiere den Prompt unten für deine Lieblings-KI",
            step2Description: "Füge die Antwort der KI hier ein",
            stepCopy: "Prompt kopieren",
            stepPaste: "Antwort einfügen",
            copyPrompt: "Prompt kopieren",
            copied: "Kopiert!",
            next: "Weiter",
            back: "Zurück",
            submit: "Rezepte importieren",
            instructions: "Klicke auf 'Prompt kopieren'. Öffne dann einen beliebigen KI-Chat (ChatGPT, Claude, Gemini, etc.), füge den Prompt ein und sende ihn ab. Wenn du die Antwort der KI hast, kopiere sie (nutze das Kopieren-Symbol",
            instructionsEnd: "unter der Antwort), komme hierher zurück und klicke auf 'Rezepte importieren'.",
            instructionsPasteIntro: "Prompt erfolgreich kopiert. Folge diesen Schritten, um die Antwort der KI zu erhalten:",
            instructionsPasteStep1: "Öffne einen beliebigen KI-Chat (ChatGPT, Claude, Gemini, etc.), füge den Prompt ein und sende ihn ab",
            instructionsPasteStep2: "Kopiere die Antwort der KI — nutze das Kopieren-Symbol",
            instructionsPasteStep2End: "unter der Antwort",
            instructionsPasteStep3: "Komme hierher zurück und klicke auf 'Rezepte importieren'",
            responsePlaceholder: "Füge hier die exakte Antwort der KI ein...",
            responseRequired: "Keine KI-Antwort in der Zwischenablage gefunden. Bitte füge sie manuell in das Feld oben ein.",
            clipboardIsPrompt: "Du hast die Antwort der KI noch nicht kopiert. Füge den Prompt in deinen KI-Chat ein, kopiere die Antwort und versuche es erneut.",
            copyFailed: "Automatisches Kopieren fehlgeschlagen. Bitte markiere den Text oben und kopiere ihn manuell."
        },
        apiKeySecurity: {
            title: "Dein API-Schlüssel wird im Browser gespeichert",
            description: "Dein API-Schlüssel wird im lokalen Speicher deines Browsers als Klartext gespeichert.",
            risks: "Das bedeutet:",
            risk1: "Jeder mit Zugang zu diesem Gerät kann deinen API-Schlüssel einsehen",
            risk2: "Browser-Erweiterungen können ihn möglicherweise auslesen",
            risk3: "Der Schlüssel bleibt gespeichert, bis du die Browserdaten löschst",
            risk4: "Der Schlüssel ist in Netzwerkanfragen sichtbar (Browser-DevTools, Netzwerkprotokolle, Proxys)",
            usage: "Der Schlüssel wird nur verwendet, um Google Gemini direkt aus deinem Browser aufzurufen — für Menüpläne, Fotoerkennung, Lagerungstipps, Bilder, Ersetzen und Chat. Keine Anfrage läuft über einen Server dieser App.",
            understand: "Verstanden, fortfahren"
        },
        apiKeyDialog: {
            heading: "Gemini-API-Schlüssel",
            description: "Der Schlüssel wird in diesem Browser gespeichert und direkt an Google Gemini gesendet. Er ermöglicht Fotoerkennung, Lagerungstipps, Rezeptbilder, das Ersetzen einzelner Rezepte und den Kochchat; mit dem Speichern wird zusätzlich die Menüplan-Generierung auf die API umgestellt, was du jederzeit zurückschalten kannst.",
            save: "Schlüssel speichern",
            delete: "Schlüssel löschen",
            errorEmpty: "Bitte gib einen API-Schlüssel ein.",
            editTooltip: "API-Schlüssel ändern",
            tooltip: "Aus: es ist kein Schlüssel gespeichert, und Menüpläne werden zum Kopieren & Einfügen vorbereitet. An: der Schlüssel wird in diesem Browser gespeichert und ermöglicht direktes Generieren, Fotoerkennung, Lagerungstipps, Rezeptbilder, das Ersetzen einzelner Rezepte und den Kochchat."
        },
        clearApiKey: {
            title: "API-Schlüssel löschen?",
            description: "Fotoerkennung, Lagerungstipps, Rezeptbilder, das Ersetzen einzelner Rezepte und der Kochchat werden dann nicht mehr angeboten, und Menüpläne entstehen wieder per Kopieren & Einfügen. Du kannst das ein paar Sekunden lang rückgängig machen.",
            clear: "Ja, löschen",
            keep: "Nein, behalten"
        },
        undo: {
            pantryEmptied: "Vorrat geleert",
            shoppingListCleared: "Einkaufsliste geleert",
            recipeDeleted: "Rezept gelöscht",
            apiKeyCleared: "API-Schlüssel gelöscht",
            mealPlanReplaced: "Neuer Menüplan erstellt",
            action: "Rückgängig"
        },
        aiDisclaimer: "Die Rezepte werden direkt von der KI generiert. Die KI kann Fehler machen. Kochen Sie immer auch mit gesundem Menschenverstand.",
        dataTransfer: {
            exportData: "Daten exportieren",
            importData: "Daten importieren",
            exportSuccess: "Daten erfolgreich exportiert!",
            importSuccess: "Daten erfolgreich importiert! Wird neu geladen...",
            invalidImportStructure: "Ungültige Sicherungsdatei. Die Dateistruktur wird nicht erkannt.",
            invalidImportJson: "Ungültige Datei. Bitte wählen Sie eine gültige JSON-Sicherungsdatei.",
            importReadError: "Die Datei konnte nicht gelesen werden. Bitte versuchen Sie es erneut.",
            importConfirm: "Dies überschreibt alle aktuellen Daten (Vorrat, Gewürze, Rezepte, Einstellungen). Fortfahren?",
        },
        sync: {
            title: "Geräte-Synchronisation",
            label: "Geräte-Sync",
            openSettings: "Sync-Einstellungen",
            enabledTooltip: "Synchronisation aktiv",
            pendingTooltip: "Änderungen werden gleich synchronisiert…",
            pushingTooltip: "Sync zu GitHub läuft…",
            pullingTooltip: "Lade von GitHub…",
            disabledTooltip: "Synchronisation ist aus",
            errorTooltip: "Sync-Fehler — siehe Sync-Einstellungen",
            tooltip: "Aus: nichts verlässt dieses Gerät. An: Rezepte, Vorrat und Einstellungen werden über einen privaten GitHub-Gist zwischen deinen Geräten abgeglichen.",
            editTooltip: "Sync-Einstellungen ändern",
            securityTitle: "Dein GitHub-Token wird im Browser gespeichert",
            securityDescription: "Für die Synchronisation zwischen Geräten wird dein GitHub Personal Access Token im lokalen Browser-Speicher im Klartext abgelegt.",
            securityRisks: "Bitte beachte:",
            securityRisk1: "Erstelle den Token ausschließlich mit dem Scope 'gist' — nicht mehr.",
            securityRisk2: "Wer Zugriff auf dieses Gerät hat, kann den Token lesen.",
            securityRisk3: "Browser-Erweiterungen können ihn möglicherweise lesen.",
            securityRisk4: "Widerrufe den Token auf GitHub, falls das Gerät verloren geht oder geteilt wird.",
            securityRecommendation: "Der Token wird nur verwendet, um einen einzigen privaten Gist mit deinen Rezeptdaten zu lesen und zu schreiben.",
            securityAccept: "Verstanden, fortfahren",
            setupHeading: "Synchronisation einrichten",
            setupDescription: "Füge einen GitHub Personal Access Token mit Scope 'gist' ein. Wenn du bereits von einem anderen Gerät aus einen Sync-Gist hast, trage seine ID ein; sonst lass das Feld leer und es wird ein neuer Gist erstellt.",
            tokenLabel: "Personal Access Token",
            tokenPlaceholder: "ghp_…",
            gistIdLabel: "Gist-ID (optional)",
            gistIdPlaceholder: "z.B. a1b2c3d4e5f6…",
            getTokenLink: "Token auf GitHub erstellen",
            createNew: "Neuen Gist erstellen",
            useExisting: "Existierenden Gist verwenden",
            creating: "Erstelle Gist…",
            connecting: "Verbinde…",
            activeHeading: "Synchronisation ist aktiv",
            activeGistId: "Gist-ID",
            copyGistId: "Gist-ID kopieren",
            gistIdCopied: "Gist-ID in die Zwischenablage kopiert",
            disableAndClear: "Ausschalten und Token löschen",
            disableAndKeep: "Ausschalten und Token behalten",
            disableNote: "Dein Gist bleibt in jedem Fall auf GitHub — das Ausschalten betrifft nur dieses Gerät. Beim Löschen verschwindet der Token endgültig aus diesem Browser: Das lässt sich nicht rückgängig machen, und zum Synchronisieren müsstest du ihn hier neu eingeben.",
            storedHeading: "Sync ist aus, Token noch gespeichert",
            storedNote: "Die Synchronisation ist auf diesem Gerät ausgeschaltet, dein GitHub-Token liegt aber weiterhin im lokalen Speicher dieses Browsers. Lösche ihn, wenn du hier nicht mehr synchronisieren willst — das lässt sich nicht rückgängig machen.",
            storedClear: "Token löschen",
            storedKeep: "Token behalten",
            tokenStoredWarning: "GitHub-Token im lokalen Speicher gespeichert",
            pulledNotification: "Von anderem Gerät synchronisiert",
            errorUnauthorized: "Sync-Fehler: Token ist ungültig oder widerrufen. Bitte aktualisieren.",
            errorNotFound: "Sync-Fehler: Der Gist wurde nicht gefunden. Er wurde möglicherweise auf GitHub gelöscht.",
            errorPayload: "Sync-Fehler: Der Gist-Inhalt ist fehlerhaft. Er wurde vielleicht außerhalb der App bearbeitet.",
            errorNetwork: "Sync-Fehler: GitHub nicht erreichbar. Bitte später erneut versuchen.",
            errorTokenEmpty: "Bitte gib einen Token ein.",
            errorGistIdEmpty: "Bitte gib eine Gist-ID ein.",
        },
    },
    French: {
        ingredients: "Ingrédients",
        instructions: "Instructions",
        needToBuy: "À Acheter",
        shoppingList: "Liste de Courses",
        appTitle: "AI Recipe Planner",
        skipToContent: "Aller au contenu principal",
        a11y: {
            expand: "Développer",
            collapse: "Réduire",
            close: "Fermer",
            info: "Info",
        },
        generate: "Générer le Plan de Repas",
        planning: "Planification des Repas...",
        cancel: "Annuler",
        cancelGeneration: "Annuler la génération du plan de repas",
        ready: "Prêt à planifier ?",
        readyDesc: "Ajoutez vos ingrédients, définissez le nombre de personnes et laissez l'IA concevoir un menu anti-gaspillage pour vous.",
        people: "Nombre de personnes",
        diet: "Régime",
        language: "Langue",
        menuTitle: "Votre Menu Personnalisé",
        pantry: "Inventaire du Garde-Manger",
        add: "Ajouter",
        remove: "Retirer",
        clickToEdit: "Cliquez pour modifier",
        noVeg: "Aucun ingrédient ajouté pour le moment.",
        apiKeyError: "Veuillez entrer une clé API valide. Obtenez-en une sur : https://aistudio.google.com/apikey",
        veggieError: "Veuillez ajouter au moins un ingrédient à votre garde-manger.",
        invalidSharedData: "Données partagées invalides. Le lien peut être corrompu.",
        generateError: "Une erreur s'est produite lors de la génération des recettes.",
        parseError: "Une erreur s'est produite lors de l'analyse de la réponse.",
        storageError: "Impossible de sauvegarder les données — le stockage de votre navigateur est peut-être plein. Certaines modifications pourraient être perdues lors de l'actualisation.",
        errors: {
            invalidStructure: "Structure de données de recette invalide",
            tryAgain: "Veuillez réessayer de générer des recettes.",
            invalidJson: "Échec de l'analyse des données de recette. L'IA a renvoyé un JSON invalide.",
            apiKeyRequired: "Clé API requise",
            fetchFailed: "Échec de la récupération des recettes",
            emptyResponse: "Aucune recette générée. L'IA a renvoyé une réponse vide.",
            timeout: "Délai d'attente dépassé. Veuillez réessayer.",
            planTimeout: "Délai d'attente dépassé. La création du plan de repas a pris trop de temps. Réessayez ou créez moins de recettes à la fois.",
            networkError: "Erreur réseau. Veuillez vérifier votre connexion Internet.",
            unexpectedError: "Une erreur inattendue s'est produite. Veuillez réessayer.",
            imageBlocked: "La génération d'image a été bloquée par le filtre de sécurité.",
            imageNoData: "Le modèle d'image n'a renvoyé aucune image. Veuillez réessayer.",
            imageQuotaExceeded: "Quota de génération d'images dépassé. Veuillez réessayer plus tard.",
            imageFreeTierUnsupported: "La génération d'images n'est pas disponible dans le palier gratuit Gemini. Les boutons d'image restent masqués jusqu'à l'enregistrement d'une autre clé.",
            chatBlocked: "L'aide-cuisine n'a pas pu répondre à cela. Reformule ta question, s'il te plaît.",
            chatQuotaExceeded: "Quota de chat épuisé. Réessaie plus tard.",
        },
        identifyIngredient: {
            buttonAriaLabel: "Identifier l'ingrédient à partir d'une photo",
            cancelAriaLabel: "Annuler l'identification",
            identifying: "Identification…",
            unknown: "Impossible d'identifier l'ingrédient — veuillez le saisir.",
            multiple: "La photo contient plusieurs ingrédients — veuillez les photographier un par un.",
            error: "Impossible d'analyser la photo. Veuillez réessayer ou saisir l'ingrédient.",
            decodeFailed: "Impossible de lire la photo sur cet appareil.",
            quotaExceeded: "Quota d'identification par photo dépassé. Veuillez réessayer plus tard.",
        },
        ownRecipe: {
            title: "Votre propre recette",
            heading: "Vos propres recettes",
            cameraAriaLabel: "Photographier une recette",
            cancelRead: "Arrêter la lecture",
            writeAriaLabel: "Saisir une recette",
            transcribing: "Lecture de la recette…",
            transcribed: "Recette lue. Veuillez la vérifier.",
            truncated: "La page était trop longue pour être lue en une fois — cette recette est incomplète. Photographiez la suite pour la compléter.",
            entryFull: "Cette entrée est pleine : la photo n'a pas été ajoutée et le texte ci-dessous est inchangé. Raccourcissez-le, puis reprenez une photo.",
            tooLongForEntry: "La recette dépasse ce qu'une entrée peut contenir et la fin manque. Répartissez-la sur deux entrées s'il vous faut le tout.",
            placeholder: "Titre, portions, ingrédients, préparation — un par ligne.",
            hint: "Elle est placée comme l'un de vos repas, recalculée pour votre nombre de personnes et conservée telle quelle, même si elle s'écarte de votre régime.",
            addPhoto: "Ajouter une photo",
            add: "Ajouter la recette",
            previewAriaLabel: "Afficher la recette",
            removeAriaLabel: "Supprimer la recette",
            unreadable: "Aucune recette détectée sur la photo. Réessayez ou saisissez-la.",
            error: "Impossible d'analyser la photo. Veuillez réessayer ou saisir la recette.",
            decodeFailed: "Impossible de lire la photo sur cet appareil.",
            quotaExceeded: "Quota de photos dépassé. Veuillez réessayer plus tard.",
        },
        photoPrivacy: {
            title: "Votre photo sera envoyée à Google",
            description: "Pour identifier un ingrédient, l'image est envoyée à Google Gemini, qui renvoie le nom de l'ingrédient.",
            beforeYouSend: "Avant d'envoyer :",
            note1: "La photo quitte votre appareil et est traitée sur les serveurs de Google.",
            note2: "Veillez à ce qu'aucune personne, aucun document ni rien d'autre ne figure sur l'image.",
            usage: "Cette application ne conserve pas la photo, mais les conditions et le traitement des données de Google s'appliquent.",
            recipeDescription: "Pour lire la recette, l'image est envoyée à Google Gemini, qui la renvoie sous forme de texte.",
            recipeNote2: "Ne photographiez que la recette — laissez de côté les notes privées, les adresses et le reste de la page.",
            send: "Envoyer la photo",
        },
        meals: "Repas à planifier",
        dietOptions: {
            vegan: "Végétalien",
            vegetarian: "Végétarien",
            mostlyVegetarian: "Principalement Végétarien",
            pescatarian: "Pescétarien",
            flexitarian: "Flexitarien",
            omnivore: "Omnivore",
            carnivore: "Carnivore",
        },
        styleWishes: "Style, Souhaits, etc.",
        styleWishesPlaceholder: "Indien, Sans gluten, ...",
        noStyleWishes: "Aucune préférence de style ajoutée pour le moment.",
        plannedRecipes: "Plats souhaités",
        plannedRecipesPlaceholder: "Risotto aux champignons, ...",
        noPlannedRecipes: "Aucun plat souhaité pour le moment — les repas sont planifiés librement.",
        spiceRack: "Épices & Produits de Base",
        spicesPlaceholder: "Ajouter épices, huiles, sauces...",
        kitchenAppliances: "Appareils de Cuisine",
        appliancesPlaceholder: "Ajouter friteuse à air, Instant Pot...",
        kitchenAppliancesInfo: "Ajoutez des appareils comme une friteuse à air ou un Vitamix. L'IA en tient compte et peut suggérer des recettes qui les utilisent.",
        noAppliances: "Aucun appareil ajouté pour le moment.",
        kitchen: {
            caption: "Ma Cuisine",
            info: "Les épices & produits de base ainsi que les appareils de cuisine appartiennent à la cuisine sélectionnée. Créez une seconde cuisine (p. ex. pour une maison de vacances) et changez ici.",
            switchAria: "Changer de cuisine",
            defaultName: "Maison",
            newKitchen: "Nouvelle cuisine…",
            namePlaceholder: "Nom de la cuisine",
            createEmpty: "Vide",
            createCopy: "Copie",
            editAria: "Modifier la cuisine",
            deleteAria: "Supprimer la cuisine",
            confirmDelete: "Supprimer ?",
            saveAria: "Enregistrer le nom",
            locationLabel: "Lieu",
            locationPlaceholder: "Ville ou localité",
            locationSearching: "Recherche…",
            locationNoResults: "Aucun lieu correspondant trouvé.",
            locationResultsOne: "lieu trouvé.",
            locationResultsMany: "lieux trouvés.",
            locationClearAria: "Supprimer le lieu",
            weatherAttribution: "Données météo : Open-Meteo (CC BY 4.0)",
            weatherTooltip: "Températures maximales des prochains jours à cet endroit. Utilisées comme suggestion discrète pour les recettes.",
            weatherChangeable: "variable",
            weatherConditions: {
                clear: "plutôt ensoleillé",
                cloudy: "plutôt nuageux",
                rain: "pluvieux",
                snow: "neigeux",
                storm: "orageux",
                fog: "brumeux",
            },
        },
        placeholders: {
            ingredient: "Ingrédient (ex: Carottes)",
            amount: "Quantité (ex: 1kg ou 3 pcs)",
        },
        pantryInfo: "Ajoutez vos ingrédients disponibles ici. L'IA essaiera de tous les utiliser dans les recettes générées.",
        spiceRackInfo: "Ajoutez des produits de base comme des épices, des huiles ou du sel. Ceux-ci orientent les recettes, mais ne seront pas ajoutés à votre liste de courses.",
        sharedPantry: "Garde-Manger Partagé",
        loadPantry: "Charger ces articles dans mon garde-manger",
        noPantryToShare: "Aucun article de garde-manger à partager !",
        openFocusView: "Ouvrir la vue focus",
        openMyPlanner: "Ouvrir mon AI Recipe Planner",
        sharePantry: "Partager le garde-manger",
        sharePantrySuccess: "URL du garde-manger copiée !",
        noSpices: "Aucune épice ajoutée pour le moment.",
        apiKeyPlaceholder: "Coller la clé API Gemini",
        apiKeyLabel: "Clé API Gemini",
        getApiKey: "Obtenir une clé API",
        languageLabel: "Sélectionner la langue",
        decreasePeople: "Diminuer le nombre de personnes",
        increasePeople: "Augmenter le nombre de personnes",
        decreaseMeals: "Diminuer le nombre de repas",
        increaseMeals: "Augmenter le nombre de repas",
        decoupled: "(découplé)",
        decoupledInfo: "Cette liste de courses a été partagée avec vous. Vos cases cochées sont enregistrées uniquement sur cet appareil et n'affectent pas votre propre liste.",
        keepScreenOn: "Garder l'écran allumé",
        screenKeptOn: "Écran maintenu allumé",
        deleteRecipe: "Supprimer la recette",
        timers: {
            title: "Minuteurs",
            startAria: "appuyez pour démarrer un minuteur",
            done: "Terminé !",
            followUp: "Temps en plus",
            pause: "Pause",
            resume: "Reprendre",
            cancel: "Annuler le minuteur",
            dismiss: "Fermer",
            mute: "Couper le son du minuteur",
            unmute: "Activer le son du minuteur",
        },
        recipeImage: {
            generate: "Générer une image",
            generating: "Génération de l'image…",
            retry: "Réessayer",
            remove: "Supprimer l'image",
        },
        replaceRecipe: {
            button: "Remplacer la recette",
            title: "Remplacer la recette",
            replacing: "Remplacement :",
            prompt: "Que préféreriez-vous à la place ?",
            placeholder: "p. ex. plus léger, avec du poulet, asiatique…",
            hint: "Laissez vide pour simplement obtenir une autre recette.",
            submit: "Remplacer",
            generating: "Recherche d'une nouvelle recette…",
            cancelGeneration: "Annuler le remplacement",
        },
        recipeChat: {
            open: "Poser une question sur cette recette",
            close: "Fermer le chat",
            title: "Aide-cuisine",
            intro: "Pose des questions sur cette recette.",
            placeholder: "Pose une question…",
            send: "Envoyer",
            clear: "Vider le chat",
            thinking: "Réflexion…",
            retry: "Réessayer",
            suggestions: [
                "Combien de temps ça prend ?",
                "À quelle température ?",
                "Des astuces de pro ?",
                "Peut-on l'affiner ?",
                "Peut-on préparer quelque chose à l'avance ?",
            ],
        },
        imageGenCost: {
            title: "Les images de recettes coûtent de l'argent",
            description: "Chaque image est dessinée par Google Gemini avec votre clé API stockée et facturée à votre compte Google.",
            beforeYouStart: "Avant la première image :",
            note1: "La génération d'images nécessite un palier Gemini payant — une clé gratuite ne le permet pas.",
            note2: "Chaque image est facturée séparément, y compris une image générée à nouveau.",
            usage: "Le titre et les ingrédients de la recette sont envoyés à Google Gemini pour dessiner l'image. Les images restent sur cet appareil : elles ne sont jamais synchronisées ni incluses dans un lien partagé.",
            generate: "Générer l'image",
        },
        emptyPantry: "Vider le garde-manger",
        clearShoppingList: "Vider la liste de courses",
        servings: { one: "portion", other: "portions" },
        nutritionPerServing: "Par portion (estimation)",
        calories: "kcal",
        carbs: "Glucides",
        fat: "Lipides",
        protein: "Protéines",
        welcome: {
            title: "Bienvenue sur AI Recipe Planner",
            intro: "Cette application vous aide à planifier vos repas avec l'IA, en minimisant le gaspillage alimentaire.",
            pantry: "Ajoutez vos ingrédients disponibles au garde-manger. L'IA essaiera de tous les utiliser.",
            spiceRack: "Le rack à épices stocke les produits de base comme les épices et huiles que vous avez toujours.",
            settings: "Choisissez vos préférences alimentaires, le nombre de personnes et de repas à planifier.",
            copyPasteMode: "Utilisez le mode Copier-Coller (par défaut) pour travailler avec n'importe quel chat IA (ChatGPT, Claude, Gemini, etc.) sans stocker d'identifiants.",
            apiKey: "Une clé API Gemini génère les plans de repas directement et débloque aussi la reconnaissance photo, les conseils de conservation, les images de recettes, le remplacement d'une recette et le chat de cuisine. Obtenez-en une sur Google AI Studio.",
            generates: "Cliquez sur 'Générer' et l'IA créera des recettes personnalisées avec une liste de courses consolidée.",
            localStorage: "Vos paramètres et recettes sont sauvegardés dans le stockage local de votre navigateur. Si vous effacez vos données de navigation, ces informations seront perdues.",
            dontShowAgain: "Ne plus afficher",
            getStarted: "Commencer"
        },
        modeSwitch: {
            label: "Génération directe",
            tooltip: "Désactivé : l'invite du plan de repas est préparée pour n'importe quel chat IA. Activé : le plan de repas est généré directement avec votre clé API Gemini stockée."
        },
        storageTips: {
            iconAriaLabel: "Conseil de conservation",
            popoverTitle: "Conseil de conservation",
            loading: "Chargement du conseil…",
        },
        copyPaste: {
            title: "Copier-Coller",
            description: "Utilisez n'importe quel chat IA au lieu d'une clé API",
            step1Description: "Copiez le prompt ci-dessous pour votre IA préférée",
            step2Description: "Collez la réponse de l'IA ici",
            stepCopy: "Copier le prompt",
            stepPaste: "Coller la réponse",
            copyPrompt: "Copier le prompt",
            copied: "Copié !",
            next: "Suivant",
            back: "Retour",
            submit: "Importer les recettes",
            instructions: "Cliquez sur 'Copier le prompt'. Ouvrez ensuite n'importe quel chat IA (ChatGPT, Claude, Gemini, etc.), collez le prompt et envoyez-le. Une fois la réponse de l'IA obtenue, copiez-la (utilisez l'icône copier",
            instructionsEnd: "sous la réponse), revenez ici et cliquez sur 'Importer les recettes'.",
            instructionsPasteIntro: "Prompt copié avec succès. Suivez ces étapes pour obtenir la réponse de l'IA :",
            instructionsPasteStep1: "Ouvrez n'importe quel chat IA (ChatGPT, Claude, Gemini, etc.), collez le prompt et envoyez-le",
            instructionsPasteStep2: "Copiez la réponse de l'IA — utilisez l'icône copier",
            instructionsPasteStep2End: "sous la réponse",
            instructionsPasteStep3: "Revenez ici et cliquez sur 'Importer les recettes'",
            responsePlaceholder: "Collez ici la réponse exacte de l'IA...",
            responseRequired: "Aucune réponse IA trouvée dans votre presse-papiers. Veuillez la coller manuellement dans le champ ci-dessus.",
            clipboardIsPrompt: "Vous n'avez pas encore copié la réponse de l'IA. Collez le prompt dans votre chat IA, copiez la réponse et réessayez.",
            copyFailed: "La copie automatique a échoué. Veuillez sélectionner le texte ci-dessus et le copier manuellement."
        },
        apiKeySecurity: {
            title: "Votre clé API sera stockée dans votre navigateur",
            description: "Votre clé API sera stockée dans le stockage local de votre navigateur en texte clair.",
            risks: "Cela signifie :",
            risk1: "Toute personne ayant accès à cet appareil peut voir votre clé API",
            risk2: "Les extensions de navigateur peuvent potentiellement la lire",
            risk3: "La clé persiste jusqu'à ce que vous effaciez les données du navigateur",
            risk4: "La clé sera visible dans les requêtes réseau (DevTools du navigateur, journaux réseau, proxys)",
            usage: "La clé sert uniquement à appeler Google Gemini directement depuis votre navigateur — pour les plans de repas, la reconnaissance photo, les conseils de conservation, les images, le remplacement et le chat. Aucune requête ne passe par un serveur de cette application.",
            understand: "Je comprends, continuer"
        },
        apiKeyDialog: {
            heading: "Clé API Gemini",
            description: "La clé est stockée dans ce navigateur et envoyée directement à Google Gemini. Elle permet la reconnaissance photo, les conseils de conservation, les images de recettes, le remplacement d'une recette et le chat de cuisine ; l'enregistrer bascule en plus la génération du plan de repas vers l'API, ce que vous pouvez annuler à tout moment.",
            save: "Enregistrer la clé",
            delete: "Supprimer la clé",
            errorEmpty: "Veuillez saisir une clé API.",
            editTooltip: "Modifier la clé API",
            tooltip: "Désactivé : aucune clé n'est stockée et les plans de repas sont préparés pour le copier-coller. Activé : la clé est stockée dans ce navigateur et permet la génération directe, la reconnaissance photo, les conseils de conservation, les images de recettes, le remplacement d'une recette et le chat de cuisine."
        },
        clearApiKey: {
            title: "Effacer la clé API ?",
            description: "La reconnaissance photo, les conseils de conservation, les images de recettes, le remplacement d'une recette et le chat de cuisine ne seront plus proposés, et les plans de repas repasseront au copier-coller. Vous pouvez annuler pendant quelques secondes.",
            clear: "Oui, effacer",
            keep: "Non, conserver"
        },
        undo: {
            pantryEmptied: "Garde-manger vidé",
            shoppingListCleared: "Liste de courses vidée",
            recipeDeleted: "Recette supprimée",
            apiKeyCleared: "Clé API effacée",
            mealPlanReplaced: "Nouveau plan de repas créé",
            action: "Annuler"
        },
        aiDisclaimer: "Les recettes sont générées directement par l'IA. L'IA peut faire des erreurs. Cuisinez toujours avec bon sens.",
        dataTransfer: {
            exportData: "Exporter les données",
            importData: "Importer les données",
            exportSuccess: "Données exportées avec succès !",
            importSuccess: "Données importées avec succès ! Rechargement...",
            invalidImportStructure: "Fichier de sauvegarde invalide. La structure du fichier n'est pas reconnue.",
            invalidImportJson: "Fichier invalide. Veuillez sélectionner un fichier de sauvegarde JSON valide.",
            importReadError: "Impossible de lire le fichier. Veuillez réessayer.",
            importConfirm: "Cela écrasera toutes vos données actuelles (garde-manger, épices, recettes, paramètres). Continuer ?",
        },
        sync: {
            title: "Synchronisation multi-appareils",
            label: "Synchronisation",
            openSettings: "Paramètres de synchronisation",
            enabledTooltip: "Synchronisation active",
            pendingTooltip: "Les modifications seront synchronisées bientôt…",
            pushingTooltip: "Synchronisation vers GitHub…",
            pullingTooltip: "Chargement depuis GitHub…",
            disabledTooltip: "Synchronisation désactivée",
            errorTooltip: "Erreur de synchronisation — voir les paramètres de synchronisation",
            tooltip: "Désactivé : rien ne quitte cet appareil. Activé : recettes, garde-manger et réglages sont synchronisés entre vos appareils via un Gist GitHub privé.",
            editTooltip: "Modifier les paramètres de synchronisation",
            securityTitle: "Votre jeton GitHub sera stocké dans votre navigateur",
            securityDescription: "Pour synchroniser entre appareils, votre jeton d'accès personnel GitHub sera stocké en clair dans le stockage local du navigateur.",
            securityRisks: "À noter :",
            securityRisk1: "Créez un jeton uniquement avec la portée 'gist' — rien de plus.",
            securityRisk2: "Toute personne ayant accès à cet appareil peut lire le jeton.",
            securityRisk3: "Les extensions de navigateur peuvent potentiellement le lire.",
            securityRisk4: "Révoquez le jeton sur GitHub si l'appareil est perdu ou partagé.",
            securityRecommendation: "Le jeton sert uniquement à lire et écrire un seul Gist privé contenant vos données de recettes.",
            securityAccept: "J'ai compris, continuer",
            setupHeading: "Configurer la synchronisation",
            setupDescription: "Collez un jeton d'accès personnel GitHub avec la portée 'gist'. Si vous disposez déjà d'un Gist de synchronisation depuis un autre appareil, entrez son identifiant ; sinon, laissez le champ vide et un nouveau Gist sera créé.",
            tokenLabel: "Jeton d'accès personnel",
            tokenPlaceholder: "ghp_…",
            gistIdLabel: "ID du Gist (optionnel)",
            gistIdPlaceholder: "ex. a1b2c3d4e5f6…",
            getTokenLink: "Créer un jeton sur GitHub",
            createNew: "Créer un nouveau Gist",
            useExisting: "Utiliser un Gist existant",
            creating: "Création du Gist…",
            connecting: "Connexion…",
            activeHeading: "Synchronisation active",
            activeGistId: "ID du Gist",
            copyGistId: "Copier l'ID du Gist",
            gistIdCopied: "ID du Gist copié dans le presse-papiers",
            disableAndClear: "Désactiver et supprimer le jeton",
            disableAndKeep: "Désactiver et conserver le jeton",
            disableNote: "Votre Gist reste sur GitHub dans tous les cas — la désactivation ne concerne que cet appareil. La suppression retire définitivement le jeton de ce navigateur : c'est irréversible, et vous devriez le saisir à nouveau pour synchroniser ici.",
            storedHeading: "Synchronisation désactivée, jeton encore stocké",
            storedNote: "La synchronisation est désactivée sur cet appareil, mais votre jeton GitHub se trouve toujours dans le stockage local de ce navigateur. Supprimez-le si vous ne comptez plus synchroniser ici — c'est irréversible.",
            storedClear: "Supprimer le jeton",
            storedKeep: "Conserver le jeton",
            tokenStoredWarning: "Jeton GitHub stocké dans le stockage local",
            pulledNotification: "Synchronisé depuis un autre appareil",
            errorUnauthorized: "Erreur de synchronisation : jeton invalide ou révoqué. Veuillez le mettre à jour.",
            errorNotFound: "Erreur de synchronisation : Gist introuvable. Il a peut-être été supprimé sur GitHub.",
            errorPayload: "Erreur de synchronisation : le contenu du Gist est mal formé. Il a peut-être été modifié hors de l'application.",
            errorNetwork: "Erreur de synchronisation : GitHub inaccessible. Veuillez réessayer plus tard.",
            errorTokenEmpty: "Veuillez saisir un jeton.",
            errorGistIdEmpty: "Veuillez saisir un ID de Gist.",
        },
    },
    Spanish: {
        ingredients: "Ingredientes",
        instructions: "Instrucciones",
        needToBuy: "Necesitas Comprar",
        shoppingList: "Lista de Compras",
        appTitle: "AI Recipe Planner",
        skipToContent: "Saltar al contenido principal",
        a11y: {
            expand: "Expandir",
            collapse: "Contraer",
            close: "Cerrar",
            info: "Info",
        },
        generate: "Generar Plan de Comidas",
        planning: "Planificando Comidas...",
        cancel: "Cancelar",
        cancelGeneration: "Cancelar la generación del plan de comidas",
        ready: "¿Listo para planificar?",
        readyDesc: "Agrega tus ingredientes, establece el número de personas y deja que la IA diseñe un menú sin desperdicios para ti.",
        people: "Personas a alimentar",
        diet: "Dieta",
        language: "Idioma",
        menuTitle: "Tu Menú Personalizado",
        pantry: "Inventario de Despensa",
        add: "Agregar",
        remove: "Eliminar",
        clickToEdit: "Haz clic para editar",
        noVeg: "No se han agregado ingredientes todavía.",
        apiKeyError: "Por favor ingresa una clave API válida. Obtén una en: https://aistudio.google.com/apikey",
        veggieError: "Por favor agrega al menos un ingrediente a tu despensa.",
        invalidSharedData: "Datos compartidos inválidos. El enlace puede estar corrupto.",
        generateError: "Algo salió mal al generar las recetas.",
        parseError: "Algo salió mal al procesar la respuesta.",
        storageError: "No se pudieron guardar los datos — el almacenamiento de su navegador puede estar lleno. Algunos cambios podrían perderse al actualizar.",
        errors: {
            invalidStructure: "Estructura de datos de receta inválida",
            tryAgain: "Por favor, intenta generar recetas de nuevo.",
            invalidJson: "Error al analizar los datos de la receta. La IA devolvió JSON inválido.",
            apiKeyRequired: "Se requiere clave API",
            fetchFailed: "Error al obtener las recetas",
            emptyResponse: "No se generaron recetas. La IA devolvió una respuesta vacía.",
            timeout: "Tiempo de espera agotado. Por favor, inténtalo de nuevo.",
            planTimeout: "Tiempo de espera agotado. La creación del plan de comidas ha tardado demasiado. Inténtalo de nuevo o crea menos recetas a la vez.",
            networkError: "Error de red. Por favor, verifica tu conexión a Internet.",
            unexpectedError: "Ocurrió un error inesperado. Por favor, inténtalo de nuevo.",
            imageBlocked: "La generación de imágenes fue bloqueada por el filtro de seguridad.",
            imageNoData: "El modelo de imagen no devolvió ninguna imagen. Por favor, inténtalo de nuevo.",
            imageQuotaExceeded: "Cuota de generación de imágenes agotada. Por favor, inténtalo más tarde.",
            imageFreeTierUnsupported: "La generación de imágenes no está disponible en el plan gratuito de Gemini. Los botones de imagen quedan ocultos hasta que guardes otra clave.",
            chatBlocked: "El ayudante no ha podido responder a eso. Reformula la pregunta, por favor.",
            chatQuotaExceeded: "Cuota de chat agotada. Inténtalo de nuevo más tarde.",
        },
        identifyIngredient: {
            buttonAriaLabel: "Identificar ingrediente desde una foto",
            cancelAriaLabel: "Cancelar identificación",
            identifying: "Identificando…",
            unknown: "No se pudo identificar el ingrediente — por favor escríbelo.",
            multiple: "La foto contiene varios ingredientes — por favor fotografía uno a la vez.",
            error: "No se pudo analizar la foto. Por favor, inténtalo de nuevo o escríbelo.",
            decodeFailed: "No se pudo leer la foto en este dispositivo.",
            quotaExceeded: "Cuota de identificación por foto agotada. Por favor, inténtalo más tarde.",
        },
        ownRecipe: {
            title: "Tu propia receta",
            heading: "Tus propias recetas",
            cameraAriaLabel: "Fotografiar una receta",
            cancelRead: "Detener la lectura",
            writeAriaLabel: "Escribir una receta",
            transcribing: "Leyendo la receta…",
            transcribed: "Receta leída. Por favor, revísala.",
            truncated: "La página era demasiado larga para leerla de una vez — esta receta está incompleta. Fotografía el resto para añadirlo.",
            entryFull: "Esta entrada está llena: la foto no se añadió y el texto de abajo no ha cambiado. Acórtalo y vuelve a fotografiar.",
            tooLongForEntry: "La receta es más larga de lo que cabe en una entrada y le falta el final. Divídela en dos entradas si la necesitas completa.",
            placeholder: "Título, raciones, ingredientes, preparación — uno por línea.",
            hint: "Se planifica como una de tus comidas, se recalcula para tu número de personas y se mantiene tal cual, aunque se aparte de tu dieta.",
            addPhoto: "Añadir una foto",
            add: "Añadir receta",
            previewAriaLabel: "Mostrar la receta",
            removeAriaLabel: "Eliminar la receta",
            unreadable: "No se detectó ninguna receta en la foto. Inténtalo de nuevo o escríbela.",
            error: "No se pudo analizar la foto. Por favor, inténtalo de nuevo o escribe la receta.",
            decodeFailed: "No se pudo leer la foto en este dispositivo.",
            quotaExceeded: "Cuota de fotos agotada. Por favor, inténtalo más tarde.",
        },
        photoPrivacy: {
            title: "Tu foto se enviará a Google",
            description: "Para identificar un ingrediente, la imagen se envía a Google Gemini, que devuelve el nombre del ingrediente.",
            beforeYouSend: "Antes de enviar:",
            note1: "La foto sale de tu dispositivo y se procesa en los servidores de Google.",
            note2: "Asegúrate de que no aparezcan personas, documentos ni nada más en la imagen.",
            usage: "Esta aplicación no almacena la foto, pero se aplican las condiciones y el tratamiento de datos de Google.",
            recipeDescription: "Para leer la receta, la imagen se envía a Google Gemini, que la devuelve como texto.",
            recipeNote2: "Fotografía solo la receta — deja fuera notas privadas, direcciones y lo demás de la página.",
            send: "Enviar foto",
        },
        meals: "Comidas a planificar",
        dietOptions: {
            vegan: "Vegano",
            vegetarian: "Vegetariano",
            mostlyVegetarian: "Mayormente Vegetariano",
            pescatarian: "Pescetariano",
            flexitarian: "Flexitariano",
            omnivore: "Omnivoro",
            carnivore: "Carnívoro",
        },
        styleWishes: "Estilo, Deseos, etc.",
        styleWishesPlaceholder: "Indio, Sin gluten, ...",
        noStyleWishes: "No se han agregado preferencias de estilo todavía.",
        plannedRecipes: "Platos deseados",
        plannedRecipesPlaceholder: "Risotto de setas, ...",
        noPlannedRecipes: "Aún no hay platos deseados — las comidas se planifican libremente.",
        spiceRack: "Especiero y Productos Básicos",
        spicesPlaceholder: "Agregar especias, aceites, salsas...",
        kitchenAppliances: "Electrodomésticos de Cocina",
        appliancesPlaceholder: "Agregar freidora de aire, Instant Pot...",
        kitchenAppliancesInfo: "Agrega electrodomésticos como una freidora de aire o Vitamix. La IA los tiene en cuenta y puede sugerir recetas que los utilicen.",
        noAppliances: "No se han agregado electrodomésticos todavía.",
        kitchen: {
            caption: "Mi Cocina",
            info: "El especiero y productos básicos y los electrodomésticos pertenecen a la cocina seleccionada. Crea una segunda cocina (p. ej. para una casa de vacaciones) y cambia aquí entre ellas.",
            switchAria: "Cambiar de cocina",
            defaultName: "Casa",
            newKitchen: "Nueva cocina…",
            namePlaceholder: "Nombre de la cocina",
            createEmpty: "Vacía",
            createCopy: "Copia",
            editAria: "Editar cocina",
            deleteAria: "Eliminar cocina",
            confirmDelete: "¿Eliminar?",
            saveAria: "Guardar nombre",
            locationLabel: "Lugar",
            locationPlaceholder: "Ciudad o localidad",
            locationSearching: "Buscando…",
            locationNoResults: "No se encontró ningún lugar.",
            locationResultsOne: "lugar encontrado.",
            locationResultsMany: "lugares encontrados.",
            locationClearAria: "Quitar lugar",
            weatherAttribution: "Datos meteorológicos: Open-Meteo (CC BY 4.0)",
            weatherTooltip: "Temperaturas máximas de los próximos días en este lugar. Se usan como sugerencia suave al planificar las recetas.",
            weatherChangeable: "cambiante",
            weatherConditions: {
                clear: "mayormente soleado",
                cloudy: "mayormente nublado",
                rain: "lluvioso",
                snow: "nevado",
                storm: "tormentoso",
                fog: "con niebla",
            },
        },
        placeholders: {
            ingredient: "Ingrediente (ej. Zanahorias)",
            amount: "Cantidad (ej. 1kg o 3 pcs)",
        },
        pantryInfo: "Agrega tus ingredientes disponibles aquí. La IA intentará usarlos todos en las recetas generadas.",
        spiceRackInfo: "Agrega productos básicos como especias, aceites o sal. Estos guían las recetas, pero no se agregarán a tu lista de compras.",
        sharedPantry: "Despensa Compartida",
        loadPantry: "Cargar estos artículos en mi despensa",
        noPantryToShare: "¡No hay artículos de despensa para compartir!",
        openFocusView: "Abrir vista de enfoque",
        openMyPlanner: "Abrir mi AI Recipe Planner",
        sharePantry: "Compartir despensa",
        sharePantrySuccess: "¡URL de despensa copiada al portapapeles!",
        noSpices: "No se han agregado especias todavía.",
        apiKeyPlaceholder: "Pegar clave API de Gemini",
        apiKeyLabel: "Clave API de Gemini",
        getApiKey: "Obtener clave API",
        languageLabel: "Seleccionar idioma",
        decreasePeople: "Disminuir número de personas",
        increasePeople: "Aumentar número de personas",
        decreaseMeals: "Disminuir número de comidas",
        increaseMeals: "Aumentar número de comidas",
        decoupled: "(desacoplado)",
        decoupledInfo: "Esta lista de compras fue compartida contigo. Tus marcas se guardan solo en este dispositivo y no afectan tu propia lista.",
        keepScreenOn: "Mantener pantalla encendida",
        screenKeptOn: "Pantalla mantenida encendida",
        deleteRecipe: "Eliminar receta",
        timers: {
            title: "Temporizadores",
            startAria: "toca para iniciar un temporizador",
            done: "¡Listo!",
            followUp: "Tiempo extra",
            pause: "Pausar",
            resume: "Reanudar",
            cancel: "Cancelar temporizador",
            dismiss: "Descartar",
            mute: "Silenciar el sonido del temporizador",
            unmute: "Activar el sonido del temporizador",
        },
        recipeImage: {
            generate: "Generar imagen",
            generating: "Generando imagen…",
            retry: "Reintentar",
            remove: "Eliminar imagen",
        },
        replaceRecipe: {
            button: "Reemplazar receta",
            title: "Reemplazar receta",
            replacing: "Reemplazando:",
            prompt: "¿Qué preferirías en su lugar?",
            placeholder: "p. ej. algo más ligero, con pollo, asiático…",
            hint: "Déjalo vacío para obtener simplemente otra receta.",
            submit: "Reemplazar",
            generating: "Buscando una nueva receta…",
            cancelGeneration: "Cancelar reemplazo",
        },
        recipeChat: {
            open: "Preguntar sobre esta receta",
            close: "Cerrar chat",
            title: "Ayudante de cocina",
            intro: "Haz preguntas sobre esta receta.",
            placeholder: "Pregunta algo…",
            send: "Enviar",
            clear: "Vaciar chat",
            thinking: "Pensando…",
            retry: "Reintentar",
            suggestions: [
                "¿Cuánto tiempo tarda?",
                "¿A qué temperatura?",
                "¿Algún consejo profesional?",
                "¿Se puede refinar?",
                "¿Se puede preparar algo con antelación?",
            ],
        },
        imageGenCost: {
            title: "Las imágenes de recetas cuestan dinero",
            description: "Cada imagen la dibuja Google Gemini con tu clave API almacenada y se cobra a tu cuenta de Google.",
            beforeYouStart: "Antes de la primera imagen:",
            note1: "La generación de imágenes requiere un plan Gemini de pago — una clave gratuita no puede hacerlo.",
            note2: "Cada imagen se cobra por separado, incluida una que vuelvas a generar.",
            usage: "El título y los ingredientes de la receta se envían a Google Gemini para dibujar la imagen. Las imágenes se quedan en este dispositivo: nunca se sincronizan ni forman parte de un enlace compartido.",
            generate: "Generar imagen",
        },
        emptyPantry: "Vaciar despensa",
        clearShoppingList: "Vaciar lista de compras",
        servings: { one: "ración", other: "raciones" },
        nutritionPerServing: "Por porción (estimado)",
        calories: "kcal",
        carbs: "Carbohidratos",
        fat: "Grasa",
        protein: "Proteína",
        welcome: {
            title: "Bienvenido a AI Recipe Planner",
            intro: "Esta app te ayuda a planificar comidas con IA, minimizando el desperdicio de alimentos.",
            pantry: "Agrega tus ingredientes disponibles a la despensa. La IA intentará usar todos ellos.",
            spiceRack: "El especiero almacena productos básicos como especias y aceites que siempre tienes a mano.",
            settings: "Elige tus preferencias dietéticas, número de personas y cuántas comidas planificar.",
            copyPasteMode: "Usa el modo Copiar y Pegar (predeterminado) para trabajar con cualquier chat de IA (ChatGPT, Claude, Gemini, etc.) sin almacenar credenciales.",
            apiKey: "Una clave API de Gemini genera los planes de comidas directamente y además desbloquea el reconocimiento de fotos, los consejos de conservación, las imágenes de recetas, el reemplazo de una receta y el chat de cocina. Obtén una en Google AI Studio.",
            generates: "Presiona 'Generar' y la IA creará recetas personalizadas con una lista de compras consolidada.",
            localStorage: "Tus configuraciones y recetas se guardan en el almacenamiento local de tu navegador. Si borras los datos del navegador, esta información se perderá.",
            dontShowAgain: "No mostrar de nuevo",
            getStarted: "Empezar"
        },
        modeSwitch: {
            label: "Generación directa",
            tooltip: "Desactivado: el prompt del plan de comidas se prepara para cualquier chat de IA. Activado: el plan de comidas se genera directamente con tu clave API de Gemini almacenada."
        },
        storageTips: {
            iconAriaLabel: "Consejo de conservación",
            popoverTitle: "Consejo de conservación",
            loading: "Obteniendo consejo…",
        },
        copyPaste: {
            title: "Copiar y Pegar",
            description: "Usa cualquier chat de IA en lugar de una clave API",
            step1Description: "Copia el prompt de abajo para usarlo con tu IA favorita",
            step2Description: "Pega la respuesta de la IA aquí",
            stepCopy: "Copiar prompt",
            stepPaste: "Pegar respuesta",
            copyPrompt: "Copiar prompt",
            copied: "¡Copiado!",
            next: "Siguiente",
            back: "Atrás",
            submit: "Importar recetas",
            instructions: "Haz clic en 'Copiar prompt'. Luego abre cualquier chat de IA (ChatGPT, Claude, Gemini, etc.), pega el prompt y envíalo. Cuando tengas la respuesta de la IA, cópiala (usa el icono de copiar",
            instructionsEnd: "debajo de la respuesta), vuelve aquí y haz clic en 'Importar recetas'.",
            instructionsPasteIntro: "Prompt copiado correctamente. Sigue estos pasos para obtener la respuesta de la IA:",
            instructionsPasteStep1: "Abre cualquier chat de IA (ChatGPT, Claude, Gemini, etc.), pega el prompt y envíalo",
            instructionsPasteStep2: "Copia la respuesta de la IA — usa el icono de copiar",
            instructionsPasteStep2End: "debajo de la respuesta",
            instructionsPasteStep3: "Vuelve aquí y haz clic en 'Importar recetas'",
            responsePlaceholder: "Pega aquí la respuesta exacta de la IA...",
            responseRequired: "No se encontró ninguna respuesta de IA en tu portapapeles. Por favor, pégala manualmente en el campo de arriba.",
            clipboardIsPrompt: "Aún no has copiado la respuesta de la IA. Pega el prompt en tu chat de IA, copia la respuesta e inténtalo de nuevo.",
            copyFailed: "No se pudo copiar automáticamente. Por favor, selecciona el texto de arriba y cópialo manualmente."
        },
        apiKeySecurity: {
            title: "Tu clave API se almacenará en tu navegador",
            description: "Tu clave API se almacenará en el almacenamiento local de tu navegador en texto plano.",
            risks: "Esto significa:",
            risk1: "Cualquier persona con acceso a este dispositivo puede ver tu clave API",
            risk2: "Las extensiones del navegador pueden leerla",
            risk3: "La clave permanece hasta que borres los datos del navegador",
            risk4: "La clave será visible en solicitudes de red (DevTools del navegador, registros de red, proxies)",
            usage: "La clave solo se usa para llamar a Google Gemini directamente desde tu navegador — para planes de comidas, reconocimiento de fotos, consejos de conservación, imágenes, reemplazo y chat. Ninguna solicitud pasa por un servidor de esta aplicación.",
            understand: "Entiendo, continuar"
        },
        apiKeyDialog: {
            heading: "Clave API de Gemini",
            description: "La clave se almacena en este navegador y se envía directamente a Google Gemini. Habilita el reconocimiento de fotos, los consejos de conservación, las imágenes de recetas, el reemplazo de una receta y el chat de cocina; al guardarla también se cambia la generación del plan de comidas a la API, algo que puedes revertir en cualquier momento.",
            save: "Guardar clave",
            delete: "Eliminar clave",
            errorEmpty: "Introduce una clave API.",
            editTooltip: "Cambiar la clave API",
            tooltip: "Desactivado: no hay ninguna clave almacenada y los planes de comidas se preparan para copiar y pegar. Activado: la clave se almacena en este navegador y habilita la generación directa, el reconocimiento de fotos, los consejos de conservación, las imágenes de recetas, el reemplazo de una receta y el chat de cocina."
        },
        clearApiKey: {
            title: "¿Eliminar clave API?",
            description: "El reconocimiento de fotos, los consejos de conservación, las imágenes de recetas, el reemplazo de una receta y el chat de cocina dejarán de ofrecerse, y los planes de comidas volverán a copiar y pegar. Puedes deshacerlo durante unos segundos.",
            clear: "Sí, eliminar",
            keep: "No, mantener"
        },
        undo: {
            pantryEmptied: "Despensa vaciada",
            shoppingListCleared: "Lista de compras vaciada",
            recipeDeleted: "Receta eliminada",
            apiKeyCleared: "Clave API eliminada",
            mealPlanReplaced: "Nuevo plan de comidas creado",
            action: "Deshacer"
        },
        aiDisclaimer: "Las recetas son generadas directamente por IA. La IA puede cometer errores. Cocine siempre con sentido común.",
        dataTransfer: {
            exportData: "Exportar datos",
            importData: "Importar datos",
            exportSuccess: "¡Datos exportados con éxito!",
            importSuccess: "¡Datos importados con éxito! Recargando...",
            invalidImportStructure: "Archivo de respaldo inválido. La estructura del archivo no es reconocida.",
            invalidImportJson: "Archivo inválido. Por favor seleccione un archivo de respaldo JSON válido.",
            importReadError: "No se pudo leer el archivo. Por favor intente de nuevo.",
            importConfirm: "Esto sobrescribirá todos sus datos actuales (despensa, especias, recetas, configuración). ¿Continuar?",
        },
        sync: {
            title: "Sincronización entre dispositivos",
            label: "Sincronización",
            openSettings: "Ajustes de sincronización",
            enabledTooltip: "Sincronización activa",
            pendingTooltip: "Los cambios se sincronizarán en breve…",
            pushingTooltip: "Sincronizando con GitHub…",
            pullingTooltip: "Cargando desde GitHub…",
            disabledTooltip: "Sincronización desactivada",
            errorTooltip: "Error de sincronización — consulta los ajustes de sincronización",
            tooltip: "Desactivado: nada sale de este dispositivo. Activado: recetas, despensa y ajustes se mantienen sincronizados entre tus dispositivos mediante un Gist privado de GitHub.",
            editTooltip: "Cambiar los ajustes de sincronización",
            securityTitle: "Tu token de GitHub se almacenará en tu navegador",
            securityDescription: "Para sincronizar entre dispositivos, su token de acceso personal de GitHub se almacenará en texto plano en el almacenamiento local del navegador.",
            securityRisks: "Tenga en cuenta:",
            securityRisk1: "Cree un token solo con el alcance 'gist' — nada más.",
            securityRisk2: "Cualquier persona con acceso a este dispositivo podrá leer el token.",
            securityRisk3: "Las extensiones del navegador podrían leerlo.",
            securityRisk4: "Revoque el token en GitHub si el dispositivo se pierde o se comparte.",
            securityRecommendation: "El token se usa únicamente para leer y escribir un solo Gist privado con los datos de sus recetas.",
            securityAccept: "Entiendo, continuar",
            setupHeading: "Configurar sincronización",
            setupDescription: "Pegue un token de acceso personal de GitHub con el alcance 'gist'. Si ya tiene un Gist de sincronización desde otro dispositivo, introduzca su ID; si no, déjelo vacío y se creará un nuevo Gist.",
            tokenLabel: "Token de acceso personal",
            tokenPlaceholder: "ghp_…",
            gistIdLabel: "ID del Gist (opcional)",
            gistIdPlaceholder: "ej. a1b2c3d4e5f6…",
            getTokenLink: "Crear un token en GitHub",
            createNew: "Crear nuevo Gist",
            useExisting: "Usar Gist existente",
            creating: "Creando Gist…",
            connecting: "Conectando…",
            activeHeading: "Sincronización activa",
            activeGistId: "ID del Gist",
            copyGistId: "Copiar ID del Gist",
            gistIdCopied: "ID del Gist copiado al portapapeles",
            disableAndClear: "Desactivar y eliminar el token",
            disableAndKeep: "Desactivar y mantener el token",
            disableNote: "Su Gist permanece en GitHub en cualquier caso: la desactivación solo afecta a este dispositivo. Al eliminarlo, el token desaparece definitivamente de este navegador, no se puede deshacer y tendría que introducirlo de nuevo para sincronizar aquí.",
            storedHeading: "Sincronización desactivada, token aún almacenado",
            storedNote: "La sincronización está desactivada en este dispositivo, pero su token de GitHub sigue en el almacenamiento local de este navegador. Elimínelo si no piensa volver a sincronizar aquí: no se puede deshacer.",
            storedClear: "Eliminar token",
            storedKeep: "Mantener token",
            tokenStoredWarning: "Token de GitHub almacenado en almacenamiento local",
            pulledNotification: "Sincronizado desde otro dispositivo",
            errorUnauthorized: "Error de sincronización: token no autorizado o revocado. Por favor actualícelo.",
            errorNotFound: "Error de sincronización: Gist no encontrado. Puede haber sido eliminado en GitHub.",
            errorPayload: "Error de sincronización: el contenido del Gist está mal formado. Puede haberse editado fuera de la aplicación.",
            errorNetwork: "Error de sincronización: no se pudo conectar con GitHub. Inténtelo más tarde.",
            errorTokenEmpty: "Por favor introduzca un token.",
            errorGistIdEmpty: "Por favor introduzca un ID de Gist.",
        },
    }
};
