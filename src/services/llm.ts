...
export const buildRecipePrompt = ({
  ...
  return `
    ...
    RECIPE STRUCTURE:
    These rules shape the "instructions" array. They govern how a method is presented, never what it cooks.
    - LEAD TIMES FIRST: anything whose latency is long relative to the work around it (preheating an oven, soaking, marinating, bringing to room temperature, proving, resting, defrosting) goes into the opening step, whatever point in the method it is chronologically needed at. Where it would then sit idle for a long stretch, put it early enough that it is ready when it is needed, and say what it is for.
    - AMOUNTS AT THE POINT OF USE: name the quantity again in the step that uses the ingredient ("stir in the 200 g of grated cheese"), so nobody has to read the ingredient list and the method at the same time. The list still carries every ingredient. Where the amount is a count rather than a measurement (one head of lettuce, a bunch of parsley, a pinch, two cloves), name the ingredient the way one would say it out loud and drop the unit: "the onion", "the bunch of mint", never "the 1 pc. onion". Where an ingredient is split across several steps, the amounts named in those steps must add up to the amount in the ingredient list.
    - PULL PREPARATION FORWARD: preparation that keeps belongs in the opening step or in an unattended window, not in the step that needs it. Toasting nuts and seeds is the case that goes wrong most often: they are a topping, so they get written last, although they can be done while something else cooks and then simply wait. The same holds for grinding spices, mixing a dressing, draining a tin, and cutting firm vegetables. What must NOT be pulled forward, because standing time ruins it: chopped leafy herbs, crushed garlic, cut fruit that browns, dressed salad, whipped egg white, an activated raising agent. Where one step bundles both kinds, split it.
    - TOTAL TIME IS WALL-CLOCK TIME: "time" is what passes between starting the first step and putting the dish on the table, with preheating, coming to the boil, soaking, marinating, proving and resting counted in. It is not the sum of the hands-on steps, and work that genuinely runs in parallel is counted once.
    - DONENESS CUE BEFORE TIME: where a step ends on a state rather than on a clock, say the state first and keep the time behind it as a guide ("reduce until it coats the back of a spoon, about 8-10 minutes"). Always keep the time: it is what the cook sets a timer on. Never give a bare time for a step whose real end is a state.
...
export const generateRecipes = async (
  ...
  const timeoutSignal = AbortSignal.timeout(API_CONFIG.PLAN_TIMEOUT_MS);
