...
export const buildRecipePrompt = ({
  ...
  return `
    ...
    11. The "ingredients" array must contain EVERY single ingredient needed for the recipe: pantry items, items to buy, and any spice rack items used in the recipe. This includes what a method mentions only in passing: fat for greasing a dish, a thickener, a splash of the cooking water, a garnish. If a step names it, it has a line in "ingredients".
    ...
    - LEAD TIMES FIRST: anything whose latency is long relative to the work around it (preheating an oven, soaking, marinating, bringing to room temperature, proving, resting, defrosting) goes into the opening step, whatever point in the method it is chronologically needed at. Where it would then sit idle for a long stretch, put it early enough that it is ready when it is needed, and say what it is for. Its duration counts towards the recipe's "time".
    - AMOUNTS AT THE POINT OF USE: name the quantity again in the step that uses the ingredient ("stir in the 200 g of grated cheese"), so nobody has to read the ingredient list and the method at the same time. The list still carries every ingredient.
